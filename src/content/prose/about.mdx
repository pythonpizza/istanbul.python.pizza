**Python Pizza**, an annual micro conference originating from Naples in 2017, has made its mark in various cities in Germany, Italy, Cuba, Czechia, Poland, as well as in remote formats. Now it's Istanbul's turn to host this amazing Python community event!

Organized entirely by volunteers, we bring the Python Community together for some amazing original Pizza™ and Python Talks.

We believe in and encourage practical sessions, in which developers share their experiences and lessons learned from real-world projects. Each talk is 10 minutes long.

Check out the past editions: [Warsaw Python Pizza](https://warsaw.python.pizza), [Ostrava Python Pizza](https://ostrava.python.pizza), [Prague Python Pizza](https://prague.python.pizza), [Brno Python Pizza](https://brno.python.pizza), [Naples Python Pizza](https://python.pizza), [Berlin Python Pizza](https://berlin.python.pizza), [2021 Hamburg Python Pizza](https://2021.hamburg.python.pizza), [2020 Hamburg Python Pizza](https://2020.hamburg.python.pizza) and [2019 Hamburg Python Pizza](https://2019.hamburg.python.pizza).
